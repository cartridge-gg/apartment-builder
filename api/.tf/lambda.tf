...
resource "aws_iam_role" "iam_for_lambda" {
  name               = "${terraform.workspace}apbuilder"
...
resource "aws_lambda_function" "lambda" {
  ...
  s3_key        = "lambda/apbuilder/${data.external.version.result["version"]}.zip"
  function_name = "${terraform.workspace}apbuilder"
  role          = aws_iam_role.iam_for_lambda.arn
  runtime       = "go1.x"
  memory_size   = 512
  timeout       = 30
  handler       = "apbuilder-linux-amd64"
